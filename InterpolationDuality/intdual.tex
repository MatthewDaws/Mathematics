...
%So there is a linear surjection $X_0^* \oplus X_1^* \to (\Delta\overline X)^\intdual$.  Given $\eta^* = (\eta_0^*,\eta_1^*) \in \Delta(\overline X^*) = (\Delta\overline X)'^\perp$, see Proposition~\ref{prop:dual}, we have that $\ip{\eta_0^*}{x_0} = \ip{\eta_1^*}{x_1}$ for all $x=(x_0,x_1) \in \Delta\overline X$, and hence 
