...
A doolittle diagram arises from a Banach couple if and only if $\sigma_0,\sigma_1$ are both injective, if and only if $\delta_0, \delta_1$ are both injective, if and only if $j$ is injective.
\end{proposition}
\begin{proof}
By the lemma, $\sigma_0$ is injective if and only if $\delta_1$ is injective, and similarly $\sigma_1$ is injective if and only if $\delta_0$ is injective.  If both $\sigma_0, \delta_0$ are injective, then so is $j = \sigma_0\delta_0$.  If $j$ is injective, then $j = \sigma_0 \delta_0 = \sigma_1 \delta_1$ is injective, so both $\delta_0, \delta_1$ are injective.

As observed, Banach couples give rise to doolittle diagrams with $\sigma_0,\sigma_1$ injective.  Conversely, if $\sigma_0,\sigma_1$ are injective, then so also are $\delta_0,\delta_1$ are injective.  As $\sigma_0,\sigma_1$ are injective, we may use $\Sigma\overline X$ as our ambient space to form $X_0\cap X_1$ and $X_0 + X_1$.  Then $\xi_0 = \xi_1$ exactly when $(\xi_0,0) = (0,\xi_1)$ in $\Sigma\overline X$, that is, $(\xi_0,\xi_1) \in D\sigma$, see Remark~\ref{rem:when_equal_in_sigma}.  Hence $X_0 \cap X_1 = D\sigma$, isometrically.  The norm on $\Sigma\overline X = P\delta$ is then, for $\xi = \xi_0+\xi_1$,
\begin{align*}
...
\subsection{Interpolation}\label{sec:interpolation_intro}
...
\begin{remark}\label{rem:factor_through_I}
...
Take $\overline A = I = J\mathbb C$, so $A$ is just a Banach space, as $\mc B(\overline A) = \mathbb C$.  As before Remark~\ref{rem:factor_through_I}, we hence see that
\[ \lan_A\overline X = \Delta\overline X \proten A, \qquad
\ran_A\overline X = \mc B((\Sigma\overline X)^*, A). \]
If we further specialise to the case when $A=\mathbb C$, we see that $\lan_A\overline X = \Delta\overline X$ and $\ran_A\overline X = (\Sigma\overline X)^{**}$.  Hence, as remarked in \cite{KP_InterpolationFunctorsDuality}, the right Kan extension is often ``too large''.


\subsection{Going forward}

I intend to stop here for the moment, so some comments on where this idea goes.  Let $\mc A_0$ be a (full) subcategory of $\msf{BanDL}$, and let $F \colon \mc A_0 \to \msf{Ban}$ be a functor.  Then $F(\overline A)$ is an $F(\overline A)$-module for each $\overline A\in\mc A_0$, that is, a candidate to be an interpolation space.  Furthermore, that $F$ is a functor implies certain compatibility conditions between the different spaces.

As in Section~\ref{sec:interpolation_intro}, a classical interpolation space for a Banach couple $\overline X = (X_0, X_1)$ is a Banach space $X$ with $X_0\cap X_1 \subseteq X \subseteq X_0+X_1$, such that every $\overline T\in\mc B(\overline X)$ gives rise to a map $T\colon X\to X$.  We recast this by saying that $X$ should be a (contractive) $\mc B(\overline X)$-module.  Furthermore, an ``interpolation method'' is, loosely, a way of assigning such a space to any Banach couple (or perhaps some subset thereof), usually with morphisms $\overline X \to \overline Y$ giving rise to maps from $X$ to $Y$.  This obviously ``looks like'' a functor, and motivates the following (terminology our own).

\begin{definition}
A (classical) \emph{interpolation scheme} is a functor $F \colon \mc A \to \msf{Ban}$ where $\mc A$ is a (full) subcategory of $\msf{BanCp}$.
\end{definition}

Recall the embedding $J \colon \msf{Ban} \to \msf{BanCp}, X \mapsto (X,X)$.  It is reasonable to suppose that when we interpolate between $X$ and itself, we simply get $X$ back.  In the language of functors, we might ask that $F\circ J = \id$.  This is perhaps too strong: we only require an (isometric) isomorphism, as we ask for in the next proposition.  Recall Definition~\ref{defn:int_space} for the notion of a quasi-interpolation space.

\begin{proposition}
Let $\mc A_0 \subseteq \msf{BanDL}$ be a full subcategory containing the image of $J$, and let $F \colon \mc A_0 \to \msf{Ban}$ be a functor such that $FJ$ is naturally isomorphic to $\id$.  Then $F(\overline X)$ is a quasi-interpolation space for $\overline X$, for each $\overline X\in\mc A_0$.
\end{proposition}
\begin{proof}
With reference to Proposition~\ref{prop:J_hom_space_adjoints}, for a doolittle diagram $\overline X$, we have that $1 \in \mc B(\Delta\overline X) \cong \mc B(J\Delta\overline X, \overline X)$ induces a map $\overline\delta = \overline\delta_{\overline X}$, and $1\in\mc B(\Sigma\overline X) \cong \mc B(\overline X, J\Sigma\overline X)$ induces a map $\overline\Sigma = \overline\Sigma_{\overline X}$.  To be more explicit, $\overline\delta \colon J\Delta\overline X \to \overline X$ is $\overline\delta = (\delta_0, \delta_1)$, and this has the property that for $\overline T\in\mc B(\overline X, \overline Y)$ and $x=(x_0,x_1)\in \Delta\overline X$, we have for $i=0,1$,
\[ (\overline T\circ\overline\delta_{\overline X})_i(x) = T_i \delta_i(x) = T_i(x_i) = \delta_i (\Delta\overline T)(x) = (\overline\delta_{\overline Y} \circ J(\Delta\overline T))_i(x). \]
Hence $\overline T\circ\overline\delta_{\overline X} = \overline\delta_{\overline Y} \circ J(\Delta\overline T)$.  Similarly, $\overline\Sigma \colon \overline X \to J\Sigma\overline X$ is $\overline\Sigma = (\sigma_0, \sigma_1)$, and with $\overline T$ as before, for $x_i\in X_i$,
\[ (\overline\Sigma_{\overline Y} \circ \overline T)_i(x_i)
= \sigma_i T_i(x_i)
= (\Sigma\overline T) \sigma_i(x_i)
= (J(\Sigma\overline T) \circ \overline\Sigma_{\overline X})_i(x_i), \]
so $\overline\Sigma_{\overline Y} \circ \overline T = J(\Sigma\overline T) \circ \overline\Sigma_{\overline X}$.

Let $\eta \colon FJ \Rightarrow \id$ be a natural isomorphism.  So we have components $\eta_X \colon FJ(X) \to X$ which are (isometric) isomorphisms, such that given $T\in\mc B(X,Y)$ we have that $T \eta_X = \eta_Y FJ(T)$.

We apply our functor to obtain a Banach space $X = F(\overline X)$ which is a $\mc B(\overline X)$-module.  Of course, $\Delta\overline X$ is always a $\mc B(\overline X)$-module, for the action $\overline T\mapsto\Delta\overline T$, and similarly $\Sigma\overline T$.  We define a map $\delta \colon \Delta\overline X \to X$ to be the composition
\[\begin{tikzcd}
\Delta\overline X \arrow[r, "\eta_{\Delta\overline X}^{-1}"]
& FJ(\Delta\overline X) \arrow[r, "F(\overline\delta)"] & F(\overline X) = X.
\end{tikzcd}\]
We show that $\delta$ is a module map.  For $\overline T\in\mc B(\overline X)$, as above, $\overline T \overline \delta = \overline\delta J(\Delta\overline T)$ and so $F(\overline T) F(\overline\delta) = F(\overline \delta) FJ(\Delta\overline T)$.  Hence $F(\overline T) \delta = F(\overline \delta) FJ(\Delta\overline T) \eta_{\Delta\overline X}^{-1} = F(\overline \delta) \eta_{\Delta\overline X}^{-1} \Delta\overline T = \delta \Delta\overline T$, which exactly says that $\delta$ is a module map.

Similarly, we define a map $\Sigma \colon X \to \Sigma\overline X$ to be the composition
\[ \begin{tikzcd}
X = F(\overline X) \arrow[r, "F(\overline\Sigma)"] & FJ(\Sigma\overline X)
\arrow[r, "\eta_{\Sigma \overline X}"] & \Sigma \overline X.
\end{tikzcd} \]
For $\overline T\in\mc B(\overline X)$ we have $\overline\Sigma \overline T = J(\Sigma\overline T) \overline\Sigma$, and so applying $F$ shows that
\[ (\Sigma\overline T) \Sigma
= (\Sigma\overline T) \eta_{\Sigma\overline X} F(\overline\Sigma)
= \eta_{\Sigma\overline X} FJ(\Sigma\overline T) F(\overline\Sigma)
= \eta_{\Sigma\overline X} F(\overline\Sigma \overline T)
= \Sigma F(\overline T). \]
We conclude that $\Sigma$ is a module map.  The maps $\delta$ and $\Sigma$ show that $X$ is a quasi-interpolation for $\overline X$.
\end{proof}

We hence naturally have the following situation.  We might have a classical interpolation scheme, expressed as a functor $F:\msf{BanCp} \to \msf{Ban}$ say.  We wish to extend this to a functor $\msf{BanDL} \to \msf{Ban}$.  This is exactly the situation of a Kan extension!

\begin{remark}
If we have that $FJ=\id$ or at least naturally isomorphic, does the Kan extension also have this property?  Surely yes, but worth writing down the details.
\end{remark}
